...
def test_git_summary_metadata_only_by_default(active_project_repo: Path) -> None:
    db = get_db()
...
def test_git_summary_include_diff_true_returns_diff(active_project_repo: Path) -> None:
...
def test_repo_list_files_glob(active_project_repo: Path) -> None:
    db = get_db()
    files = call_tool(db, name="reos_repo_list_files", arguments={"glob": "src/reos/*.py"})
    assert "src/reos/example.py" in files


def test_repo_read_file_line_range(active_project_repo: Path) -> None:
    db = get_db()
    out = call_tool(
        db,
...
def test_repo_read_file_blocks_escape(active_project_repo: Path) -> None:
    db = get_db()
...
def test_repo_read_file_range_too_large(active_project_repo: Path) -> None:
    db = get_db()
    with pytest.raises(ToolError) as exc:
        call_tool(
            db,
...
def test_repo_grep_finds_matches(active_project_repo: Path) -> None:
    db = get_db()
...
def test_tools_require_active_project_repo(isolated_db_singleton) -> None:  # noqa: ANN001
    # No active project configured in this DB.
    db = get_db()

    with pytest.raises(ToolError) as exc:
        call_tool(db, name="reos_git_summary", arguments={})
    assert exc.value.code == "no_active_project_repo"
